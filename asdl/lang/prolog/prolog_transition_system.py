...
from asdl.asdl_ast import RealizedField, AbstractSyntaxNode
...
def prolog_node_to_ast(grammar, prolog_tokens, start_idx):
    ...
    if node_name in ['job', 'language', 'loc', 'req_deg', 'application', 'area', 'company',
        ...
        ast_node = AbstractSyntaxNode(prod, [pred_field, arg_field])
    elif node_name in ['ANS', 'X', 'A', 'B', 'P', 'J']:
        # it's a variable
        prod = grammar.get_prod_by_ctr_name('Variable')
        ast_node = AbstractSyntaxNode(prod,
...
def prolog_expr_to_ast_helper(grammar, prolog_tokens, start_idx=0):
    ...
    while True:
        ...
        if prolog_tokens[i] == ',':
            ...
            if len(parsed_nodes) == 1:
                ...
                left_ast_node = AbstractSyntaxNode(left_expr_prod, [RealizedField(left_expr_prod['arguments'], parsed_nodes)])
                parsed_nodes = []
    ...
    if len(parsed_nodes) > 1:
        ...
        return_node = AbstractSyntaxNode(prod, [RealizedField(prod['arguments'], parsed_nodes)])
    else:
...
def is_equal_ast(this_ast, other_ast):
    ...
    if isinstance(this_ast, AbstractSyntaxNode):
